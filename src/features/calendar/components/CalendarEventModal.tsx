...
import { type CalendarEvent, type CalendarEventType } from "../utils/getSeasonEvents";
...
> = {
  ...
  birthday: { icon: "/images/ui/cake.png", label: "Cumpleaños", color: "text-pink-600 bg-pink-50/50 border-pink-100" },
  ...
  librero: { icon: "/images/events/librero.webp", label: "Vendedor de Libros", color: "text-amber-700 bg-amber-50 border-amber-200" },
  vendor: { icon: "/images/events/traveling.webp", label: "Comerciante", color: "text-emerald-700 bg-emerald-50 border-emerald-200" },
  crop: { icon: "/images/events/star.png", label: "Cultivo", color: "text-green-700 bg-green-50 border-green-200" },
};
...
const SEASON_THEMES: Record<string, { bg: string, border: string, text: string, button: string, buttonHover: string, badgeBg: string, badgeBorder: string, giftBg: string, giftBorder: string, giftText: string }> = {
  spring: { 
    bg: "bg-emerald-50", 
...
export function CalendarEventModal({ event, onClose }: CalendarEventModalProps) {
  ...
  const modalBorder = birthdayTheme ? birthdayTheme.border : (BORDER_COLORS[colorBase] || BORDER_COLORS.stone);
  const modalText = birthdayTheme ? birthdayTheme.text : (TEXT_COLORS[colorBase] || TEXT_COLORS.stone);
    ...
    >
      <div className="flex flex-col gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className={`inline-block rounded-full border px-2.5 py-0.5 text-xs font-semibold shadow-sm ${isBirthday && birthdayTheme ? `${birthdayTheme.text} ${birthdayTheme.badgeBg} ${birthdayTheme.badgeBorder}` : config.color}`}>
              {config.label}
            </span>
        ...
        <div className="rounded-xl border border-stone-200 bg-white/60 backdrop-blur-sm divide-y divide-stone-200 shadow-sm overflow-hidden">
          ...
          {isBirthday && villager && birthdayTheme && (
            <div className="px-4 py-3">
              <p className="text-[11px] font-semibold uppercase tracking-wide text-stone-400 mb-2">🎁 Regalos Favoritos</p>
              <div className="flex flex-wrap gap-1.5">
                {villager.lovedGifts.map((gift) => (
                  <span key={gift} className={`rounded-lg px-2.5 py-1 text-[11px] font-medium border ${birthdayTheme.giftBg} ${birthdayTheme.giftText} ${birthdayTheme.giftBorder}`}>
                    {gift}
                  </span>
                ))}
              </div>
