...
type CalendarItem =
  ...
  | { type: "vendor"; data: { name: string; icon: string; vendorData: CalendarEvent } };
  ...
  return (
    ...
    >
      ...
      <div className="absolute right-1.5 top-1 z-10">
        <p className="text-[10px] font-bold leading-none text-amber-950/40 sm:text-xs sm:text-amber-950/60">
          {day}
        </p>
      </div>
      ...
      <div className="mt-4 grid w-full grid-cols-2 gap-1.5 sm:mt-6 sm:gap-3">
        {allItems.map((item, index) => {
          ...
          const shouldSpan = allItems.length <= 2 || (allItems.length === 3 && index === 0);
